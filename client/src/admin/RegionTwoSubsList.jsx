...
const RegionTwoSubsList = () => {
  ...
  const subscriptionData = async () => {
    try {
      ...
      if (response.success) {
        setData(response.data.data.filter((elem) => elem.status==="Approved" && elem.region_name === 2));
      } else {
        ...
        <div>
          <div className="box_header">
            <div style={{ padding: "5px 0px" }}>
              <i className="fa-regular fa-rectangle-list mx-3"></i>&nbsp; Total Region-2 Subscriptions:   {data.length}
            </div>
          </div>
          <div>
...
export default RegionTwoSubsList
